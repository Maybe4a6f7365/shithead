...
export class Room{
 private sessions=new Map<string,Session>();private data:Data|null=null
 constructor(private state:DurableObjectState,private env:Env){}
 async fetch(req:Request){const p=new URL(req.url).pathname
  if(p.endsWith('/ws')){if(!allowed(req.headers.get('Origin')))return new Response('Forbidden origin',{status:403});if(req.headers.get('Upgrade')?.toLowerCase()!=='websocket')return new Response('Upgrade required',{status:426});return this.ws(req)}
  return new Response('Not found',{status:404})}
 private ws(req:Request){const pair=new WebSocketPair(),client=pair[0],server=pair[1],sid=crypto.randomUUID();this.sessions.set(sid,{webSocket:server,playerId:null});server.accept();server.addEventListener('message',e=>this.msg(sid,String(e.data)));server.addEventListener('close',()=>this.close(sid));server.addEventListener('error',()=>this.close(sid));return new Response(null,{status:101,webSocket:client})}
 private msg(sid:string,raw:string){const s=this.sessions.get(sid);if(!s)return;let m:ClientMsg;try{const x=JSON.parse(raw);if(!isClientMsg(x))throw 0;m=x}catch{return this.send(s,{type:'ERROR',code:'INTERNAL',message:'Bad message'})}
  switch(m.type){case'CREATE_ROOM':return this.create(s,m.playerName,m.maxPlayers);case'JOIN_ROOM':return this.join(s,m.code,m.playerName);case'LEAVE_ROOM':return this.leave(s);case'START_GAME':return this.start(s);case'READY':return this.ready(s);case'REARRANGE':return this.swap(s,m.handIdx,m.upIdx);case'PLAY':return this.play(s,m.cards);case'PICK_UP':return this.pick(s);case'CHAT':return this.chat(s,m.text);case'PING':return this.send(s,{type:'PONG',ts:Date.now()})}}
 private create(s:Session,name:string,max=5){if(this.data)return this.send(s,{type:'ERROR',code:'INTERNAL',message:'Room already exists'});name=name.trim();if(!name||name.length>32)return this.send(s,{type:'ERROR',code:'INTERNAL',message:'Invalid name'});const p=mkPlayer(name);s.playerId=p.id;this.data={code:new URL(s.webSocket.url||'https://x').pathname.split('/')[3]||'',hostId:p.id,maxPlayers:Math.max(2,Math.min(5,max)),players:[p],state:null,createdAt:Date.now()};this.welcome(s);this.room()}
 private join(s:Session,code:string,name:string){const d=this.data;name=name.trim();if(!d||d.code!==code.toUpperCase())return this.send(s,{type:'ERROR',code:'INVALID_CODE',message:'Room not found'});if(d.players.length>=d.maxPlayers)return this.send(s,{type:'ERROR',code:'ROOM_FULL',message:'Room full'});if(d.state)return this.send(s,{type:'ERROR',code:'INTERNAL',message:'Game already started'});const p=mkPlayer(name);s.playerId=p.id;d.players.push(p);this.welcome(s);this.room()}
...
export default{async fetch(req:Request,env:Env){const u=new URL(req.url),p=u.pathname;if(req.method==='OPTIONS')return new Response(null,{headers:cors(req.headers.get('Origin'))});if(p==='/api/health')return new Response('OK',{headers:cors(req.headers.get('Origin'))});if(p==='/api/room/new'&&req.method==='POST'){const roomId=code();return Response.json({roomId},{headers:cors(req.headers.get('Origin'))})}const m=p.match(/^\/api\/room\/([A-Z0-9]{6})\/ws$/);if(m){const id=env.ROOM.idFromName(m[1]);return env.ROOM.get(id).fetch(req)}if(env.ASSETS){const r=await env.ASSETS.fetch(req);if(r.status!==404)return r;if(req.method==='GET'&&!p.startsWith('/api/'))return env.ASSETS.fetch(new Request(new URL('/index.html',req.url)))}return new Response('Not found',{status:404,headers:cors(req.headers.get('Origin'))})}}
